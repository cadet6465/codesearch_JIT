...
def evaluate(args, model, tokenizer, checkpoint=None, prefix="", mode='dev'):
    ...
    for eval_task, eval_output_dir in zip(eval_task_names, eval_outputs_dirs):
        ...
        pred2= torch.nn.functional.softmax(torch.tensor(preds),dim=1)
        pred2 = pred2.numpy()
        result, result2 = compute_metrics(eval_task, preds_label, out_label_ids,pred2)
        ...
        if (mode == 'dev'):
            ...
            with open(output_eval_file, "a+") as writer:
                ...
                logger.info("  %s = %s", 'AUC', str(result2['auc']))
                writer.write("%s = %s\n" % ('AUC', str(result2['auc'])))
...
def main():
    ...
    parser.add_argument("--data_dir", default='./codesearch_data/train_valid/test/', type=str,
                        help="The input data dir. Should contain the .tsv files (or other data files) for the task.")

    parser.add_argument("--output_dir", default='./models/test', type=str,
    ...
    parser.add_argument('--save_steps', type=int, default=50,
                        help="Save checkpoint e_tvery X updates steps.")
    ...
    parser.add_argument("--pred_model_dir", default='./models/test/checkpoint-best', type=str,
